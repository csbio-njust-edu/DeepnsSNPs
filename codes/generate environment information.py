...
##----------------       主函数  for MMP_dataset_D_mut -- PSA -----------------##
filename = get_filename("../data/PSA/MMP_dataset_D_mut")  ## 从得到的特征文件中获取filename

## 从原始文件中获取 POS,ID,label，便于后面提取突变点微环境的特征
POS,ID,label = get_POS_Name_Label('../data/MMP_dataset_D_mut.xlsx','deleterious subset')

##从得到的特征文件夹中，获得相应的文件，并提取突变点的微环境特征，IDname, Labelname
feature,IDName,Labelname = get_fea_IDN_LabelN("../data/PSA/MMP_dataset_D_mut",filename,POS,ID,label,4)

## 保存得到的特征文件,第二个参数是特征的列名称，第三个参数是列名称的长度
save_file("../data/PSA/MMP_dataset_D_mut_PSA_9.xlsx",'psa',27)  
