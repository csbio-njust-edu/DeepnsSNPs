PRE_PSPP_9 = open('./result/PRE_CNN_9_27_PSPP.txt','w')
...
数据集分为 train 和 test后，就直接从下面load_train 和 test开始
## 数据集分为 train 和 test后，就直接从下面load_train 和 test开始
...
PRE_PSPP_9.write('\n\n\n--------- PSPP (PSSM + Secondary structure + PDO + PSA )-----------\n\n\n')
...
import numpy as np
Y_pred = model.predict(X_test,batch_size=batch_size, verbose=1)
##对于二类来说，结果类似与[[0.8,0.8],[0.1,0.9]]
p = np.array(Y_pred)
i = 1 # 取第一列
temp = p[:, i] ## 取其中一维，这里取第一维
Y_pred = temp.tolist() ##
